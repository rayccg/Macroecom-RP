...
Unemployment rate and nominal GDP data were collected as outcome variables while tax revenue, public gross fixed capital formation (i.e., public investment) and government social assistance payments (i.e., social subsidies) are the explanatory variables of interest. Government final consumption is used as control variables to disaggregate the effects of arguable less productive government spending. Lastly, non-tax revenue, which is less or has a different distortionary effect from the market than taxes and key monetary policy variables are also used a control variables to avoid omitted variable issues.
...
Across variables, ACF plots show strong, positive, and gradually decaying autocorrelation structures. Moreover the PACF plots indicate a near-one value at the first lag and much lower, mostly statistically zero values at higher lag orders. These characteristics may be indicating that many variables may be following a random walk with drift process and as such may be unit-root non-stationary.
...
The reduced form of the model allows for estimation where $B_0^{-1}$ is the matrix of contemporaneous effects between variables.

The study will utilize impulse response functions (IRF) and forecast error variance decomposition (FEVD) methods to measure the effects of the tax policy, public investment, and government social transfers on the unemployment rate and GDP output. IRFs are used to measure the dynamic, marginal effects of orthogonal shocks from the three fiscal levers of interest while FEVD intends to account for the share of variability in the outcome variables that can be emperically attributed to explanatory variables of interest.
